...
function generateMockTasks(goal, startDate, userId, journeyId) {
  ...
  const descriptions = [
    ...
    `Try a new healthy habit for a day.`,
    `Spend 10 minutes on a creative activity.`,
    `Do a random act of kindness.`,
    `Plan your week ahead and set intentions.`,
    `Take a mindful walk and reflect on your goals.`,
    `Write a letter to your future self.`,
    `Celebrate your progress with a small reward.`
  ];
  return descriptions.slice(0, 14).map((desc, i) => ({
    journeyId,
    userId,
    day: i + 1,
    description: desc,
...
export const generateJourney = async (req, res) => {
  try {
    ...
    // Only use up to 14 tasks, filter out invalid ones
    const validTasks = tasks
      .filter(t => t && typeof t.description === 'string' && t.description.trim().length > 0)
      .slice(0, 14); // Hard cap at 14 tasks
